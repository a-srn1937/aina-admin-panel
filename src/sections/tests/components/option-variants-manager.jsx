...
import { useGetLanguages, useAddOptionVariants } from 'src/api';
...
const optionVariantsSchema = z.object({
  variants: z.array(
    z.object({
...
export function OptionVariantsManager({ optionId, existingVariants = [], onSuccess }) {
  const { data: languagesData } = useGetLanguages();
  const languages = languagesData || [];

  const { mutateAsync: addVariants, isPending } = useAddOptionVariants();
  ...
  const methods = useForm({
    defaultValues,
    resolver: zodResolver(optionVariantsSchema),
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      await addVariants({ id: optionId, variants: formattedVariants });
      toast.success('ترجمه‌های گزینه با موفقیت ذخیره شدند');
      onSuccess?.();
    } catch (error) {
      console.error(error);
      toast.error('خطا در ذخیره ترجمه‌های گزینه');
    }
  });
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      <Stack spacing={3} sx={{ pt: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="subtitle1">مدیریت ترجمه‌های گزینه</Typography>
          <Button
            size="small"
